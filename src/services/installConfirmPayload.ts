 ...
 *
 * 数据流：视图/重试入口 → installGate.confirmMarketInstall → installConfirmPayload(entry, mode, ver)
 * 构造载荷 → linkdesk.dialog.confirmContent({ pluginId:"marketplace", viewId:"marketplace-install-confirm",
 * payload }) → 壳解析声明视图 renderPath → 池 DialogHost content 槽挂载 ConfirmInstall 视图 → 视图经
 ...
 *
 * E6#71k 修订（2026-09-10 用户拍板「都问」）：**每次安装/更新都弹这张卡，官方目录也不例外**，
 * 卡片不再携带任何「信任语义」（旧的 `trustGrant: remember | never` 已删——不再记忆任何来源）。
 * `mode` 携带本次动作类别，决定标题/按钮文案（安装 vs 更新）；`plaintext` 是来源协议的展示提示。
 */
...
/** 本次确认的动作类别——安装与更新共用同一张卡，只有文案不同（E6#71k 修订 D1） */
export type ConfirmMode = "install" | "update";

/** 来源是否 http 明文（无 TLS）——卡片据此加一句提示。目录源经 normalizeSourceUrl 归一为
 *  https，本判据正常恒 false；保留 = 万一将来放行非 GitHub 明文源，卡片不得静默少一句。 */
...
export interface InstallConfirmPayload {
  ...
  size?: number;
  license?: string;
  /** 本次动作类别——视图据此选标题/按钮文案（不接收成品文本——硬约束 2） */
  mode: ConfirmMode;
  /** 来源 http 明文——卡片加一句提示；正常目录恒 false（见 isHttpSourceUrl） */
  plaintext?: boolean;
}

/**
 * 构造安装确认载荷——字段 = 目录条目子集 + 动作类别 + 实际目标版本。
...
export function installConfirmPayload(
  ...
  return {
    ...
    mode,
    plaintext: isHttpSourceUrl(entry.sourceUrl),
  };
}
